...
extern void andro_timidity_config_print(const char *tag, const char *text, const char *params);
extern void andro_timidity_config_int_print(const char *text, const int params);
#endif
...
#if defined(main) || defined(ANOTHER_MAIN) || defined ( IA_W32GUI ) || defined ( IA_W32G_SYN ) || defined (__ANDROID_API__)
...
static int parse_val_float_t(FLOAT_T *param, const char *arg, FLOAT_T low, FLOAT_T high,
                             const char *name);

static inline int set_val_float_t(FLOAT_T *param, FLOAT_T i, FLOAT_T low, FLOAT_T high,
                                  const char *name);
...
static Quantity **config_parse_modulation(const char *name, int line, const char *cp, int *num,
                                          int mod_type) {
    const char *p, *px, *err;
    char buf[128], *delim;
    Quantity **mod_list;
    ...
    static const uint16 qtypes[] = {
            QUANTITY_UNIT_TYPE(TREMOLO_SWEEP), QUANTITY_UNIT_TYPE(TREMOLO_RATE),
            QUANTITY_UNIT_TYPE(DIRECT_INT),
            QUANTITY_UNIT_TYPE(VIBRATO_SWEEP), QUANTITY_UNIT_TYPE(VIBRATO_RATE),
            QUANTITY_UNIT_TYPE(DIRECT_INT)
    };
    ...
    for (i = 0, p = cp; i < *num; i++, p++) {
        ...
        for (j = 0; j < 3; j++, p++) {
            ...
            if ((delim = strpbrk(strncpy(buf, p, sizeof buf - 1), ":,")) != NULL)
                *delim = '\0';
            if (*buf != '\0' &&
                (err = string_to_quantity(buf, &mod_list[i][j], qtypes[mod_type * 3 + j])) !=
                NULL) {
                ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
                          "%s: line %d: %s: parameter %d of item %d: %s (%s)",
...
static int set_patchconf(char *name, int line, ToneBank *bank, char *w[], int dr, int mapid,
                         int bankmapfrom, int bankno) {
    int i;
...
static int mapname2id(char *name, int *isdrum) {
    ...
    found = (MapNameEntry *) bsearch(name, data, sizeof data / sizeof data[0], sizeof data[0],
                                     mapnamecompare);
    if (found != NULL) {
        *isdrum = found->isdrum;
        return found->mapid;
...
static int read_config_file(char *name, int self, int allow_missing_file) {
                ...
                if (!isspace(terminator[1]) && terminator[1] != '\0') {
                    ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
                              "%s: line %d: there must be at least one whitespace between "
                                      "string terminator (%c) and the next parameter", name, line,
                              tmp[i]);
                    CHECKERRLIMIT;
                    param_parse_err = 1;
                    break;
	 ...
	 */
	    ...
	    *cp++ = '\0';
		...
		continue;
            ...
            char *sf_file;
            andro_timidity_log_print("TIMIDITY", "cfg sf!");
    ...
    reuse_mblock(&varbuf);
    close_file(tf);
    andro_timidity_log_print("TIMIDITY", "cfg error99");
...
static inline int parse_opt_delay(const char *arg) {
    ...
    if ((p = strchr(arg, ',')) != NULL) if ((effect_lr_delay_msec = atoi(++p)) < 0) {
...
MAIN_INTERFACE int timidity_post_load_configuration(void) {
    ...
    if (!got_a_configuration) {
        if (try_config_again && !read_config_file(
#ifdef __ANDROID_API__
                getConfig2()
#else
                CONFIG_FILE
                ...
                , 0, 0))
            got_a_configuration = 1;
        andro_timidity_log_print("TIMIDITY", "error1 config : %s", getConfig2());
        andro_timidity_log_print("TIMIDITY", "error1");
    }
        ...
        if (config_string_list != NULL) {
            for (i = 0; config_string_list[i]; i++) {
                if (!read_config_file(config_string_list[i], 1, 0)) {
                    got_a_configuration = 1;
                    andro_timidity_log_print("TIMIDITY", "error2");
                } else {
                    cmderr++;
                }
            }
...
MAIN_INTERFACE int timidity_play_main(int nfiles, char **files) {
    ...
    init_load_soundfont();
    andro_timidity_log_print("TIMIDITYY", "success load soundfont!");
    if (!output_fail) {
...
/* Try to continue if it is Windows version */
    ...
    if (nfiles > 0)
        files_nbuf = files[0];
#if !defined(IA_W32GUI) && !defined(IA_W32G_SYN) && !defined(__ANDROID_API__)
    if (dumb_error_count)
		sleep(1);
#endif
#ifdef IA_W32GUI
