...
#define SPEED_CHANGE_RATE        1.0293022366434920 /* 2^(1/24) */
//#define SPEED_CHANGE_RATE		1.0145453349375236 /* 2^(1/48) */
/* Undefine if you don't want to use auto voice reduce implementation */
#define REDUCE_VOICE_TIME_TUNING    (play_mode->rate*2) /* 0.2 sec */
//#define REDUCE_VOICE_TIME_TUNING (96000) // 96000 always works.
...
int volatile stream_max_compute = 500;
/* compute time limit (in msec) when streaming */
...
/* release envelope control */
int opt_delay_control = 0;
/* CC#94 delay(celeste) effect control */
int opt_eq_control = 0;
/* channel equalizer control */
int opt_insertion_effect = 0;
/* insertion effect control */
int opt_drum_effect = 0;
/* drumpart effect control */
int32 opt_drum_power = 100;
/* coef. of drum amplitude */
int opt_amp_compensation = 0;
int opt_modulation_envelope = 0;
int opt_pan_delay = 0;
/* phase difference between left ear and right ear. */
int opt_user_volume_curve = 0;
int opt_default_module = MODULE_TIMIDITY_DEFAULT;
int opt_preserve_silence = 1;
...
char *pcm_alternate_file = NULL;
/* NULL or "none": Nothing (default)
				  * "auto": Auto select
				  * filename: Use it
				  */
...
/* Length of event_list */
int32 current_sample;
/* Number of calclated samples */
...
static int mainvolume_max;
/* maximum value of mainvolume */
static double compensation_ratio = 1.0;
...
static void reset_controllers(int c) {
    ...
    if (prescanning_flag) {
        if (channel[c].volume > mainvolume_max) {    /* pick maximum value of mainvolume */
            ...
            ctl->cmsg(CMSG_INFO, VERB_DEBUG, "ME_MAINVOLUME/max (CH:%d VAL:%#x)", c,
                      mainvolume_max);
        }
    }
...
void reset_midi(int playing) {
    ...
    for (i = 0; i < MAX_CHANNELS; i++) {
		 ...
		 */
        if (die && channel[i].forced_instrument)
            channel[i].forced_instrument = 0;
        if (die && channel[i].forced_volume)
            channel[i].forced_volume = 0;


        if (!channel[i].forced_instrument)
    ...
    if (voice[v].vibrato_control_ratio || channel[ch].mod.val > 0) {
        ...
        if (channel[ch].mod.val > 0) {
            if (vp->vibrato_control_ratio == 0) {
                ...
                vp->orig_vibrato_control_ratio = (int) (cnv_Hz_to_vib_ratio(5.0) *
...
static void recompute_amp(int v) {
    ...
    if (opt_user_volume_curve) {
        ...
        tempamp = master_volume *
                  voice[v].sample->volume *
                  gm2_vol_table[calc_velocity(ch, voice[v].velocity)] *
                  /* velocity: not in GM2 standard */
...
void recompute_voice_filter(int v) {
    ...
    if (opt_modulation_envelope) {
        if (voice[v].sample->tremolo_to_fc + (int16) depth_cent) {
            cent += ((double) voice[v].sample->tremolo_to_fc + depth_cent) *
                    lookup_triangular(voice[v].tremolo_phase >> RATE_SHIFT);
        }
...
static int32 calc_random_delay(int ch, int note) {
    ...
    if (bank->tone[nprog].rnddelay == 0) { return 0; }
        ...
        return (int32) ((double) bank->tone[nprog].rnddelay * play_mode->rate / 1000.0
...
static int select_play_sample(Sample *splist,
    ...
    for (i = 0, sp = splist; i < nsp; i++, sp++) {
        ...
        if ((sf = sp->scale_factor) != 1024) {
            ...
            ft = f, fst = fs;
        if (ISDRUMCHANNEL(ch) && channel[ch].drums[kn] != NULL) if (
                (ratio = get_play_note_ratio(ch, kn)) != 1.0)
    ...
    if (opt_pan_delay && channel[ch].insertion_effect == 0 && !opt_surround_chorus) {
        if (vp->panning == 64) { vp->delay += pan_delay_table[64] * play_mode->rate / 1000; }
            ...
            if (pan_delay_table[vp->panning] > pan_delay_table[127 - vp->panning]) {
                ...
                vp->delay +=
                        (pan_delay_table[vp->panning] - pan_delay_diff) * play_mode->rate / 1000;
            } else {
                pan_delay_diff = pan_delay_table[127 - vp->panning] - pan_delay_table[vp->panning];
                vp->delay +=
                        (pan_delay_table[127 - vp->panning] - pan_delay_diff) * play_mode->rate /
                        1000;
            }
...
void midi_program_change(int ch, int prog) {
    ...
    int newbank, b, p, map;
    if (ch &
        0x8000) // Clear the instrument lock. There should never be more than 0xFF channels right?
    {
        channel[ch & 0x7FF].forced_instrument = 0;
        return;
    } else if (ch & 0x800) {
        channel[ch & 0x7FF].forced_instrument = 1;
    }
    if ((channel[ch & 0x7FF].forced_instrument && (ch & 0x800)) ||
        (!channel[ch & 0x7FF].forced_instrument && !(ch & 0x800)) || prescanning_flag) {
        ch &= 0x7FF;
 ...
 */
        ...
        if (channel[ch].forced_instrument) {
            //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "Programming %d",prog);
            ctl_prog_event(ch);
        }
    }
}

void midi_volume_change(int ch, int vol) {
    if (ch &
        0x8000) // Clear the instrument lock. There should never be more than 0xFF channels right? Heck, 32 should be the max
    {
        //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "Unlocking volume");
        channel[ch & 0x7FF].forced_volume = 0;
        return;
    } else if (ch & 0x800) {
        //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "Locking volume");
        channel[ch & 0x7FF].forced_volume = 1;
    }
    if ((channel[ch & 0x7FF].forced_volume && (ch & 0x800)) ||
        (!channel[ch & 0x7FF].forced_volume && !(ch & 0x800)) || prescanning_flag) {
        //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "Changing volume: %d", vol);
        ch &= 0x7FF;
        channel[ch].volume = vol;
        adjust_volume(ch);
        ctl_mode_event(CTLE_VOLUME, 1, ch, vol);
    }
}
        ...
        switch (b) {
            ...
            case 0x26:    /* Pitch Offset Fine */
                channel[ch].pitch_offset_fine =
                        (FLOAT_T) ((((int32) val << 4) | (int32) val) - 0x80) / 10.0;
                ctl->cmsg(CMSG_INFO, VERB_NOISY, "Pitch Offset Fine (CH:%d %3fHz)", ch,
                          channel[ch].pitch_offset_fine);
                break;
            case 0x27:    /* Insertion Effect Parameter */
                ...
                insertion_effect_gs.type = ((int32) insertion_effect_gs.type_msb << 8) |
            ...
            case 0x28:    /* Insertion Effect Parameter */
                ...
                if (temp == insertion_effect_gs.type) {
                    ...
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "EFX TYPE (%02X %02X)",
        ...
        if (note == 3 && msb == 0) {    /* Effect 2 */
            ...
            switch (b) {
                case 0x00:    /* Insertion Effect Type MSB */
                    if (insertion_effect_xg[note].type_msb != val) {
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "Insertion Effect Type MSB (%d %02X)",
                                  note, val);
                ...
                case 0x01:    /* Insertion Effect Type LSB */
                    if (insertion_effect_xg[note].type_lsb != val) {
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "Insertion Effect Type LSB (%d %02X)",
                                  note, val);
                ...
                case 0x0B:
                    ...
                    temp = b - 0x02;
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Insertion Effect Parameter %d (%d %d)",
                              temp + 1, note, val);
                ...
                case 0x0D:    /* MW Insertion Control Depth */
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "MW Insertion Control Depth (%d %d)", note,
                              val);
                ...
                case 0x0E:    /* BEND Insertion Control Depth */
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "BEND Insertion Control Depth (%d %d)", note,
                              val);
                ...
                case 0x0F:    /* CAT Insertion Control Depth */
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "CAT Insertion Control Depth (%d %d)", note,
                              val);
                ...
                case 0x10:    /* AC1 Insertion Control Depth */
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "AC1 Insertion Control Depth (%d %d)", note,
                              val);
                ...
                case 0x11:    /* AC2 Insertion Control Depth */
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "AC2 Insertion Control Depth (%d %d)", note,
                              val);
                ...
                case 0x12:    /* CBC1 Insertion Control Depth */
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "CBC1 Insertion Control Depth (%d %d)", note,
                              val);
                ...
                case 0x13:    /* CBC2 Insertion Control Depth */
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "CBC2 Insertion Control Depth (%d %d)", note,
                              val);
                ...
                case 0x42:
                    if (!insertion_effect_xg[note].use_msb) { break; }
                    temp = (b - 0x30) / 2;
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Insertion Effect Parameter %d MSB (%d %d)",
                              temp + 1, note, val);
                ...
                case 0x43:
                    if (!insertion_effect_xg[note].use_msb) { break; }
                    temp = (b - 0x31) / 2;
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Insertion Effect Parameter %d LSB (%d %d)",
                              temp + 1, note, val);
                    if (insertion_effect_xg[note].param_lsb[temp] != val) {
                        insertion_effect_xg[note].param_lsb[temp] = val;
                        recompute_effect_xg(&insertion_effect_xg[note]);
            ...
            switch (b) {
                ...
                case 0x02:    /* EQ frequency1 */
                    if (opt_eq_control) {
                        if (val > 60) { val = 60; }
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ frequency1 (%d Hz)",
                ...
                case 0x06:    /* EQ frequency2 */
                    if (opt_eq_control) {
                        if (val > 60) { val = 60; }
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ frequency2 (%d Hz)",
                                  (int32) eq_freq_table_xg[val]);
                ...
                case 0x0A:    /* EQ frequency3 */
                    if (opt_eq_control) {
                        if (val > 60) { val = 60; }
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ frequency3 (%d Hz)",
                                  (int32) eq_freq_table_xg[val]);
                ...
                case 0x0E:    /* EQ frequency4 */
                    if (opt_eq_control) {
                        if (val > 60) { val = 60; }
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ frequency4 (%d Hz)",
                                  (int32) eq_freq_table_xg[val]);
                ...
                case 0x12:    /* EQ frequency5 */
                    if (opt_eq_control) {
                        if (val > 60) { val = 60; }
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ frequency5 (%d Hz)",
                                  (int32) eq_freq_table_xg[val]);
            ...
            switch (b) {
                ...
                case 0x06:    /* Same Note Number Key On Assign */
                    if (val == 0) {
                        ...
                        ctl->cmsg(CMSG_INFO, VERB_NOISY,
                                  "Same Note Number Key On Assign: Single (CH:%d)", ch);
                    } else if (val == 1) {
                        channel[ch].assign_mode = 2;
                        ctl->cmsg(CMSG_INFO, VERB_NOISY,
                                  "Same Note Number Key On Assign: Multi (CH:%d)", ch);
                    } else if (val == 2) {
                        ctl->cmsg(CMSG_INFO, VERB_NOISY,
                                  "Same Note Number Key On Assign: Inst is not supported. (CH:%d)",
                                  ch);
                    }
                    break;
                case 0x11:    /* Dry Level */
...
static void play_midi_prescan(MidiEvent *ev) {
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "Prescan resetting");
    reset_midi(0);
            ...
            for (k = offset; k < offset + 16; k++) {
                ...
                switch (ev->type) {
                    ...
                    case ME_RESET:
                        ...
                        //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "me_reset1");
                        reset_midi(0);
                        break;
                    ...
                    case ME_MAINVOLUME:
                        if (ev->a > mainvolume_max) {
                            mainvolume_max = ev->a;
                            ctl->cmsg(CMSG_INFO, VERB_DEBUG, "ME_MAINVOLUME/max (CH:%d VAL:%#x)",
                                      ev->channel, ev->a);
                        }
                        break;
                }
...
static void update_rpn_map(int ch, int addr, int update_now) {
    ...
    switch (addr) {
        ...
        case NRPN_ADDR_0130:    /* EQ BASS */
            if (opt_eq_control) {
                ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ BASS (CH:%d %.2f dB)", ch,
        ...
        case NRPN_ADDR_0131:    /* EQ TREBLE */
            if (opt_eq_control) {
                ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ TREBLE (CH:%d %.2f dB)", ch,
                          0.19 * (double) (val - 0x40));
        ...
        case NRPN_ADDR_0134:    /* EQ BASS frequency */
            if (opt_eq_control) {
                ...
                ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ BASS frequency (CH:%d %d Hz)", ch,
                          (int32) eq_freq_table_xg[val]);
        ...
        case NRPN_ADDR_0135:    /* EQ TREBLE frequency */
            if (opt_eq_control) {
                ...
                ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ TREBLE frequency (CH:%d %d Hz)", ch,
                          (int32) eq_freq_table_xg[val]);
        ...
        case RPN_ADDR_0005:        /* GM2: Modulation Depth Range */
            channel[ch].mod.lfo1_pitch_depth = (((int32) channel[ch].rpnmap[RPN_ADDR_0005] << 7) |
                                                channel[ch].rpnmap_lsb[RPN_ADDR_0005]) * 100 / 128;
            ctl->cmsg(CMSG_INFO, VERB_NOISY,
    ...
    while (MIDI_EVENT_TIME(current_event) < until_time) {
        ...
        for (j = 0; j < MAX_CHANNELS; j += 16) {
            ...
            for (k = offset; k < offset + 16; k++) {
                ...
                switch (current_event->type) {
                    ...
                    case ME_SOFT_PEDAL:
                        if (opt_lpf_def) {
                            ...
                            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Soft Pedal (CH:%d VAL:%d)", ch,
                    ...
                    case ME_HARMONIC_CONTENT:
                        if (opt_lpf_def) {
                            ...
                            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Harmonic Content (CH:%d VAL:%d)", ch,
                    ...
                    case ME_BRIGHTNESS:
                        if (opt_lpf_def) {
                            ...
                            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Brightness (CH:%d VAL:%d)", ch,
                    ...
                    case ME_REVERB_EFFECT:
                        if (opt_reverb_control) {
                            if (ISDRUMCHANNEL(ch) && get_reverb_level(ch) !=
                                                     current_event->a) { channel[ch].drum_effect_flag = 0; }
                    ...
                    case ME_CHORUS_EFFECT:
                        if (opt_chorus_control == 1) {
                            if (ISDRUMCHANNEL(ch) && channel[ch].chorus_level !=
                                                     current_event->a) { channel[ch].drum_effect_flag = 0; }
                        ...
                        if (current_event->a) {
                            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Chorus Send (CH:%d LEVEL:%d)", ch,
                                      current_event->a);
                        }
                        break;

                    case ME_TREMOLO_EFFECT:
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "Tremolo Send (CH:%d LEVEL:%d)", ch,
                                  current_event->a);
                        break;

                    case ME_CELESTE_EFFECT:
                        if (opt_delay_control) {
                            if (ISDRUMCHANNEL(ch) && channel[ch].delay_level !=
                                                     current_event->a) { channel[ch].drum_effect_flag = 0; }
                            channel[ch].delay_level = current_event->a;
                            if (play_system_mode == XG_SYSTEM_MODE) {
                                ctl->cmsg(CMSG_INFO, VERB_NOISY, "Delay Send (CH:%d LEVEL:%d)", ch,
                                          current_event->a);
                            } else {
                                ctl->cmsg(CMSG_INFO, VERB_NOISY, "Variation Send (CH:%d LEVEL:%d)",
                                          ch, current_event->a);
                            }
                        }
                        break;
                    ...
                    case ME_PHASER_EFFECT:
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "Phaser Send (CH:%d LEVEL:%d)", ch,
                                  current_event->a);
                        break;
                    ...
                    case ME_RESET:
                        ...
                        //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "me_reset2");
                        reset_midi(0);
                        break;
                    ...
                    case ME_SYSEX_GS_LSB:
                        process_sysex_event(ME_SYSEX_GS_LSB, ch, current_event->a,
                                            current_event->b);
                        break;

                    case ME_SYSEX_GS_MSB:
                        process_sysex_event(ME_SYSEX_GS_MSB, ch, current_event->a,
                                            current_event->b);
                        break;

                    case ME_SYSEX_XG_LSB:
                        process_sysex_event(ME_SYSEX_XG_LSB, ch, current_event->a,
                                            current_event->b);
                        break;

                    case ME_SYSEX_XG_MSB:
                        process_sysex_event(ME_SYSEX_XG_MSB, ch, current_event->a,
                                            current_event->b);
                        break;
                ...
                if (realjumping) {
                    ctl_prog_event(ch);
                    ctl_mode_event(CTLE_VOLUME, 1, ch, channel[ch].volume);
                }
#ifndef SUPPRESS_CHANNEL_LAYER
            }
        }
...
static void skip_to(int32 until_time) {
    ...
    change_system_mode(DEFAULT_SYSTEM_MODE);
    //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "skip to jump");
    reset_midi(0);
    buffered_count = 0;
...
static int apply_controls(void) {
    ...
    do {
        switch (rc = ctl->read(&val)) {
            ...
            case RC_TOGGLE_PAUSE:
#ifndef __ANDROID_API__ // This is derpy on android
                if(play_pause_flag)
	    {
		play_pause_flag = 0;
	    ...
	    jump_flag = 1;
            ...
            case RC_SPEEDDOWN:
                ...
                sync_restart(0);
                midi_time_ratio *= r;
                sample_count = (int32) (sample_count * r +
                                        0.5); // We get more samples as we slow down. The original timidity++ didn't have this
                current_sample = (int32) (current_sample * r + 0.5);
                trace_offset(current_sample);
                jump_flag = 1;
                ctl_mode_event(CTLE_TIME_RATIO, 0, 100 / midi_time_ratio + 0.5, 0);
...
static void do_compute_data_midi(int32 count) {
    ...
    /* is EQ valid? */
    channel_eq =
            opt_eq_control && (eq_status_gs.low_gain != 0x40 || eq_status_gs.high_gain != 0x40 ||
                               play_system_mode == XG_SYSTEM_MODE);
        ...
        if (opt_insertion_effect) {    /* insertion effect */
            for (i = 0; i < XG_INSERTION_EFFECT_NUM; i++) {
                if (insertion_effect_xg[i].part <= MAX_CHANNELS) {
                    do_insertion_effect_xg(vpblist[insertion_effect_xg[i].part], cnt,
                                           &insertion_effect_xg[i]);
                }
            }
            for (i = 0; i < XG_VARIATION_EFFECT_NUM; i++) {
                if (variation_effect_xg[i].part <= MAX_CHANNELS) {
                    do_insertion_effect_xg(vpblist[variation_effect_xg[i].part], cnt,
                                           &variation_effect_xg[i]);
                }
            }
        }
...
int play_event(MidiEvent *ev) {
    ...
    if (cet > current_sample) {
        ...
        if (midi_streaming != 0) if ((cet - current_sample) * 1000 / play_mode->rate
                                     > stream_max_compute) {
            kill_all_voices();
            /* reset_voices(); */
            /* ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY,
        ...
        for (l = offset; l < offset + 16; l++) {
            ...
            switch (ev->type) {
                ...
                case ME_LEGATO_FOOTSWITCH:
                    ...
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Legato Footswitch (CH:%d VAL:%d)", ch,
                              channel[ch].legato);
                    break;
                ...
                case ME_REVERB_EFFECT:
                    if (opt_reverb_control) {
                        if (ISDRUMCHANNEL(ch) &&
                            get_reverb_level(ch) != ev->a) { channel[ch].drum_effect_flag = 0; }
                ...
                case ME_CHORUS_EFFECT:
                    if (opt_chorus_control) {
                        if (opt_chorus_control == 1) {
                            if (ISDRUMCHANNEL(ch) && channel[ch].chorus_level !=
                                                     ev->a) { channel[ch].drum_effect_flag = 0; }
                ...
                case ME_CELESTE_EFFECT:
                    if (opt_delay_control) {
                        if (ISDRUMCHANNEL(ch) &&
                            channel[ch].delay_level != ev->a) { channel[ch].drum_effect_flag = 0; }
                        channel[ch].delay_level = ev->a;
                        if (play_system_mode == XG_SYSTEM_MODE) {
                            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Variation Send (CH:%d LEVEL:%d)", ch,
                                      ev->a);
                        } else {
                            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Delay Send (CH:%d LEVEL:%d)", ch,
                                      ev->a);
                ...
                case ME_RESET:
                    change_system_mode(ev->a);
                    //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "me_reset3: %x",ev->type);
                    reset_midi(1);
                    break;
...
static int play_midi(MidiEvent *eventlist, int32 samples) {
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "wrd");
    reset_midi(0);
    if (!opt_realtime_playing &&
        ...
        //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "prescann sett again");
        reset_midi(0);
    }
...
static int play_midi_load_file(char *fn,
    ...
    *event = read_midi_file(tf, &nevents, nsamples, fn);
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "%d supported events, %d samples, time %d:%02d", nevents, *nsamples, *nsamples/play_mode->rate/60, (*nsamples / play_mode->rate) % 60);
    ctl->cmsg(CMSG_INFO, VERB_NOISY,
...
int play_midi_file(char *fn) {
    andro_timidity_log_print("TIMIDITY", "play midi file : ");
    int i, j, rc;
	...
	{
	    andro_timidity_log_print( "TIMIDITY", "return rc line 8525");
    	return rc;
	}
#endif
    ...
    andro_timidity_log_print( "TIMIDITY", "play midi file 1");
    CLEAR_CHANNELMASK(channel_mute);
    andro_timidity_log_print( "TIMIDITY", "play midi file 2");
    if (temper_type_mute & 1)
        FILL_CHANNELMASK(channel_mute);
    andro_timidity_log_print( "TIMIDITY", "play midi file 3");
    ...
    andro_timidity_log_print( "TIMIDITY", "play midi file 4");
    ctl_mode_event(CTLE_METRONOME, 0, 0, 0);
    andro_timidity_log_print( "TIMIDITY", "play midi file 411");
    ctl_mode_event(CTLE_KEYSIG, 0, current_keysig, 0);
    andro_timidity_log_print( "TIMIDITY", "play midi file 412");
    ctl_mode_event(CTLE_TEMPER_KEYSIG, 0, 0, 0);
    andro_timidity_log_print( "TIMIDITY", "play midi file 413");
    ctl_mode_event(CTLE_KEY_OFFSET, 0, note_key_offset, 0);
    andro_timidity_log_print( "TIMIDITY", "play midi file 41");
    i = current_keysig + ((current_keysig < 8) ? 7 : -9), j = 0;
    andro_timidity_log_print( "TIMIDITY", "play midi file 42");
    while (i != 7)
        i += (i < 7) ? 5 : -7, j++;
    andro_timidity_log_print( "TIMIDITY", "play midi file 43");
    j += note_key_offset, j -= floor(j / 12.0) * 12;
    current_freq_table = j;
    andro_timidity_log_print( "TIMIDITY", "play midi file 45");
    ctl_mode_event(CTLE_TEMPO, 0, current_play_tempo, 0);
    andro_timidity_log_print( "TIMIDITY", "play midi file 46");
    ctl_mode_event(CTLE_TIME_RATIO, 0, 100 / midi_time_ratio + 0.5, 0);
    andro_timidity_log_print( "TIMIDITY", "play midi file 47");
    for (i = 0; i < MAX_CHANNELS; i++) {
        andro_timidity_log_print( "TIMIDITY", "play midi file 8");
    ...
    andro_timidity_log_print( "TIMIDITY", "play midi file 5");
    rc = play_midi_load_file(fn, &event, &nsamples);
    andro_timidity_log_print( "TIMIDITY", "play midi file 6");
    if (RC_IS_SKIP_FILE(rc))
...
void playmidi_stream_init(void) {
    ...
    for (i = 0; i < MAX_CHANNELS; i++)
        memset(channel[i].drums, 0, sizeof(channel[i].drums));
    change_system_mode(DEFAULT_SYSTEM_MODE);
    //__android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "dfsmode");
    reset_midi(0);
