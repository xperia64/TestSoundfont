...
void andro_timidity_config_print(const char *tag, const char *text, const char *params) {
    __android_log_print(ANDROID_LOG_ERROR, tag, text, params);
}

void andro_timidity_config_int_print(const char *text, const int params) {
    __android_log_print(ANDROID_LOG_ERROR, "TIMIDITY", text, params);
}
...
static jmethodID pushBufferWrite;
...
static jmethodID maxTimeInitId;
static jmethodID updateTime;
...
Java_com_example_testsoundfont_JNIHandler_prepareTimidity(JNIEnv *env, jobject obj, jstring config,
                                                          ...
                                                          jint jfreeInsts) {
    __android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "prepareTimidity");
    itIsDone = 0;
    if (!jreloading) {
        Android_JNI_SetupThread();
        //jclass tmp =
        ...
        pushBufferWrite = (*env)->GetStaticMethodID(env, pushClazz, "buffitWrite", "([BI)V");
        ...
        maxTimeInitId = (*env)->GetStaticMethodID(env, pushClazz, "initMaxtime", "(I)V");
        updateTime = (*env)->GetStaticMethodID(env, pushClazz, "updateTime", "(II)V");
...
Java_com_example_testsoundfont_JNIHandler_loadSongTimidity(JNIEnv *env, jobject obj, jstring song) {
    andro_timidity_log_print("TIMIDITY", "load song timidity");
...
Java_com_example_testsoundfont_JNIHandler_setResampleTimidity(JNIEnv *env, jobject obj,
                                                              jint jcustResamp) {
    andro_timidity_log_print("TIMIDITY", "set resample");
...
Java_com_example_testsoundfont_JNIHandler_decompressSFArk(JNIEnv *env, jobject obj, jstring jfrom,
    ...
    andro_timidity_log_print("TIMIDITY", "decompress sfark");
    jboolean isCopy;
...
Java_com_example_testsoundfont_JNIHandler_setChannelTimidity(JNIEnv *env, jobject obj, jint jchan,
                                                             jint jprog) {
    andro_timidity_log_print("TIMIDITY", "set channel");
...
Java_com_example_testsoundfont_JNIHandler_setChannelVolumeTimidity(JNIEnv *env, jobject obj,
    ...
    andro_timidity_log_print("TIMIDITY", "set channel volume");
...
char *getConfig2() {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "getConfig2 configFile : %s", configFile);
    __android_log_print(ANDROID_LOG_DEBUG, "TIMIDITY", "getConfig2 configFile2 : %s", configFile2);
    return configFile2;
}

int getFreeInsts() {
    andro_timidity_log_print("TIMIDITY", "FreeInsets");
...
int nativePush(char *buf, int nframes) {
    ...
    (*theGoodEnv)->CallStaticVoidMethod(theGoodEnv, pushClazz, pushBufferWrite, byteArr, nframes);
    (*theGoodEnv)->DeleteLocalRef(theGoodEnv, byteArr);

    andro_timidity_log_print("TIMIDITY", "nativePush");
...
Java_com_example_testsoundfont_JNIHandler_controlTimidity(JNIEnv *env, jobject obj, jint jcmd,
                                                          jint jcmdArg) {
    andro_timidity_log_print("TIMIDITY", "controlTimidity");
    (*dr_rc) = (int) jcmd;
    (*dr_arg) = (int) jcmdArg;
...
Java_com_example_testsoundfont_JNIHandler_timidityReady(JNIEnv *env, jobject obj) {
    andro_timidity_log_print("TIMIDITY", "timidity ready");
    return ((*dr_rc) ? JNI_FALSE : JNI_TRUE);
}

void flushIt() {
    andro_timidity_log_print("TIMIDITY", "flush it");
    JNIEnv *theGoodEnv = Android_JNI_GetEnv();
    (*theGoodEnv)->CallStaticIntMethod(theGoodEnv, pushClazz, flushId);
}

int getBuffer() {
    andro_timidity_log_print("TIMIDITY", "get buffer");
    JNIEnv *theGoodEnv = Android_JNI_GetEnv();
...
int getMono() {
    andro_timidity_log_print("TIMIDITY", "get mono");
    return mono;
}

int getSixteen() {
    andro_timidity_log_print("TIMIDITY", "get sixteen");
    return sixteen;
}
...
void setMaxTime(int time) {
    andro_timidity_log_print("TIMIDITY", "set max time");
    JNIEnv *theGoodEnv = Android_JNI_GetEnv();
    (*theGoodEnv)->CallStaticVoidMethod(theGoodEnv, pushClazz, maxTimeInitId, time);
}

void setCurrTime(int time, int v) {
    andro_timidity_log_print("TIMIDITY", "current time");
    JNIEnv *theGoodEnv = Android_JNI_GetEnv();
    (*theGoodEnv)->CallStaticVoidMethod(theGoodEnv, pushClazz, updateTime, time, v);
}

void controller(int aa) {
    andro_timidity_log_print("TIMIDITY", "controller");
    JNIEnv *theGoodEnv = Android_JNI_GetEnv();
    (*theGoodEnv)->CallStaticVoidMethod(theGoodEnv, pushClazz, controlId, aa);
}

int getSampleRate() {
    andro_timidity_log_print("TIMIDITY", "get sample rate");
    JNIEnv *theGoodEnv = Android_JNI_GetEnv();
    return (*theGoodEnv)->CallStaticIntMethod(theGoodEnv, pushClazz, rateId);
}

void setCurrLyric(char *lyric) {
    andro_timidity_log_print("TIMIDITY", "set current lyric");
    JNIEnv *theGoodEnv = Android_JNI_GetEnv();
